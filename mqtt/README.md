...
MQTT broker dùng để kết nối **VMSign Agent** (máy người dùng) với **Web/App SDK** (server ký số) qua mạng internet, thay thế giao thức HTTP chỉ hoạt động cùng mạng LAN.

## Kiến trúc

```
┌──────────────────┐         ┌─────────────────────┐         ┌──────────────────┐
│  Web/App SDK     │         │   MQTT Broker        │         │ VMSign Agent  │
...
|-------|-------|----------|
| `vmsignagent/{agentId}/status` | Agent → Broker (retained) | Trạng thái online + danh sách chứng thư số |
| `vmsignagent/{agentId}/sign/req` | SDK → Agent | Yêu cầu ký (hash SHA-256) |
| `vmsignagent/{agentId}/sign/res` | Agent → SDK | Kết quả ký (chữ ký số) |
| `vmsignagent/+/status` | SDK subscribe (wildcard) | Khám phá tất cả agent online |

## Luồng hoạt động

1. **Agent khởi động** → kết nối broker, publish retained presence (danh sách cert trên token)
2. **SDK discover** → subscribe `vmsignagent/+/status`, thu thập agent nào đang online
3. **SDK gửi yêu cầu ký** → publish `vmsignagent/{agentId}/sign/req` với hash + serial cert
4. **Agent ký** → nhận request, ký bằng PKCS#11, publish response tới `vmsignagent/{agentId}/sign/res`
...
| `vmsign-agent` | `VMSignAgent@Sign2024` | Tất cả VMSign Agent (dùng chung) |
...
```json
{
  "Mqtt": {
    ...
    "BrokerPort": 8883,
    "Username": "vmsign-agent",
    "Password": "VMSignAgent@Sign2024",
    "AgentId": "",
    "UseTls": true,
    "CaCertPath": "",
    "ClientPfxPath": "",
...
- **ACL**: `sdk-server` chỉ đọc status + sign; `vmsign-agent` chỉ write status + sign/res
